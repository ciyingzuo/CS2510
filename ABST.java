...
class Node<T> extends ABST<T> {
    ...
    public ABST<T> insert(Node<T> n) {
        ...
        if (Math.abs(heightDiff) > 1) {
            // :O Our tree is not balanced, time to get to work
            if (heightDiff > 1) {
                /*
                 * these casts are safe since if the height difference
                 * is greater than 1, then the right side must be a node
                 */
                ...
                if (heightDiff < 0) {
                    // in case of imbalance on left side of the right
                    // we rotate the rightsubtree of the right subtree right
                    ...
                    // in case of imbalance on right side of the right
                ...
                /*
                 * these casts are safe since if the height difference
                 * is greater than 1, then the left side must be a node
                 */
                Node<T> subtreeLeft = (Node<T>)this.left;
                heightDiff = subtreeLeft.right.getHeight() -
                        subtreeLeft.left.getHeight();
                if (heightDiff > 0) {
                    // in case of imbalance on right side of the left
                    // we rotate the leftsubtree of the left subtree right
                    // and then rotate left
                    /*
                     *        a            a            c    
                     *       /            /            / \   
    ...
    public ABST<T> insert(T t) {
        return this.insert(new Node<T>(t
    ...
    public boolean sameTree(ABST<T> n) {
        if (n.isLeaf()) {
            ...
            return this.left.sameTree((((Node<T>)n).left)) &&
                    this.right.sameTree((((Node<T>)n).right));
        }
        else { 
            return false;
        }
    ...
    public boolean sameData(ABST<T> n) {
        return order.compare(((Node<T>)this.getLeftmostHelp()).data
                , ((Node<T>)n.getLeftmostHelp()).data) == 0 &&
                this.getRight().sameData(n.getRight());
    }
    public int getHeight() {
        return this.height;
    }
...
class ExamplesABST {
    Book book1 = new Book("a", "g1", 1);
    Book book2 = new Book("b", "f1", 2);
    Book book3 = new Book("c", "e1", 5);
    Book book4 = new Book("d", "d1", 7);
    Book book5 = new Book("e", "c1", 6);
    Book book6 = new Book("f", "b1", 4);
    Book book7 = new Book("g", "a1", 3);
    ...
    boolean testIsLeaf(Tester t) {
        ...
        return t.checkExpect(tLeaf.isLeaf(), true) &&
                t.checkExpect(aLeaf.isLeaf(), true) &&
                t.checkExpect(pLeaf.isLeaf(), true) &&
    ...
    boolean testInsert(Tester t) {
        ...
        return t.checkExpect(titleTree, titleCheck) &&
                t.checkExpect(authorTree, authorCheck) &&
    ...
    boolean testGetHeight(Tester t) {
        ...
        return t.checkExpect(titleT1.getHeight(), 1) &&
                t.checkExpect(tLeaf.getHeight(), 0) &&
                t.checkExpect(titleT2.getHeight(), 2) &&
                t.checkExpect(titleT4.getHeight(), 3);
    }
    ...
    boolean testGetLeftmost(Tester t) {
        ...
        return t.checkException(new RuntimeException("No leftmost item of an empty tree"),
                tLeaf,
                "getLeftmostHelp") &&
                t.checkExpect(titleT4.getLeftmostHelp(), titleT1);
    }
    ...
    boolean testSameData(Tester t) {
        ABST<Book> BSTA = new Node<Book>(book3
                , new Node<Book>(book2
                        , new Node<Book>(book1, tLeaf, tLeaf, title)
                        , tLeaf
                        , title)
                , new Node<Book>(book4, tLeaf, tLeaf, title)
                , title);
        ABST<Book> BSTB = new Node<Book>(book3
                ...
                , new Node<Book>(book4, tLeaf, tLeaf, title)
                , title);
        ABST<Book> BSTC = new Node<Book>(book2
                , new Node<Book>(book1, tLeaf, tLeaf, title)
                , new Node<Book>(book4
                        , new Node<Book>(book3, tLeaf, tLeaf, title)
                        , tLeaf
                        , title)
                , title);
        ABST<Book> BSTD = new Node<Book>(book3
                , new Node<Book>(book1, tLeaf, tLeaf, title)
                , new Node<Book>(book4
                        , tLeaf
                        , new Node<Book>(book5, tLeaf, tLeaf, title)
                        , title)
                , title);
    ...
    boolean testBuildList(Tester t) {
        ...
        IList<Book> ListA = new Cons<Book>(book1,
                new Cons<Book>(book2,
                        new Cons<Book>(book3,
                                new Cons<Book>(book4, new Empty<Book>()))));
    ...
    boolean testBuildTreefromList(Tester t) {
        ...
        return t.checkExpect(new Empty<Book>().buildTree(new Leaf<Book>(title))
                , new Leaf<Book>(title))
                && t.checkExpect(new Empty<Book>().buildTree(BSTA), BSTA)
                && t.checkExpect(list1.buildTree(BSTB).sameData(BSTA.insert(book5)), true);
    }
}